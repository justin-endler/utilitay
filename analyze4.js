  ...
  async.eachSeries(fileNames, function(fileName, callback) {
    ...
    lineReader.on('line', function (line) {
      if (lineIndex % 2 === 0) {
        ...
        let favMlRaw = parseInt10(fav[6]);
        let favMl = Math.abs(favMlRaw);
        ...
        // data has cases where there is no positive ML, that's strange, skip it
        if (otherMl < 0 && favMlRaw < 0) {
          r = 0;
          h = 0;
        }
        if (r || h) {
